...
export function ComentariosCertificaciones({ solicitudId, isAdmin }: { solicitudId?: string; isAdmin?: boolean }) {
  ...
  const fetchComentarios = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        `id, usuario_id, comentario, fecha, respuesta_a, usuario_nomina!inner(colaborador, avatar_path, genero)`
      )
  ...
  // Fetch contador mensajes sin leer
  ...
  useEffect((): (() => void) => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "comentarios_certificacion", filter: `solicitud_id=eq.${solicitudId}` },
        () => {
          fetchComentarios()
          fetchUnreadCount()
        }
    ...
    <>
       ...
       {user && (
         <div className="border-t pt-4">
           <div className="flex gap-3">
             <img src={user.avatarUrl} alt={user.nombre} className="h-10 w-10 rounded-full" />
             <div className="flex-1">
               <div className="bg-gray-50 rounded-lg p-3">
                 <Textarea placeholder="¿Qué estás pensando, administrador?" value={nuevoComentario} onChange={e => setNuevoComentario(e.target.value)} className="resize-none border-0 bg-transparent focus-visible:ring-0" rows={2} />
                 <div className="flex justify-between items-center mt-3">
                   <div className="relative">
                     <button onClick={() => setShowEmojiPicker(v => !v)} className="p-2 rounded-full hover:bg-gray-200"><Smile className="h-5 w-5 text-gray-500" /></button>
                     {showEmojiPicker && (
                       <div className="absolute top-full left-0 mt-2 bg-white border rounded-lg shadow-lg p-3 z-50 w-72">
                         <div className="flex justify-between items-center mb-3 pb-2 border-b">
                           <span className="font-medium">Emojis</span>
                           <button onClick={() => setShowEmojiPicker(false)}><X className="h-4 w-4 text-gray-500" /></button>
                         </div>
                         <div className="grid grid-cols-8 gap-2">
                           {commonEmojis.map(emoji => (<button key={emoji} onClick={() => insertEmoji(emoji)} className="text-2xl p-2 hover:bg-gray-100 rounded transition-colors">{emoji}</button>))}
                         </div>
                       </div>
                     )}
                   </div>
                   <Button onClick={() => setShowConfirmModal(true)} disabled={loading || !nuevoComentario.trim()} className="bg-[#441404] hover:bg-[#5a1a05]"><Send className="h-4 w-4 mr-2" />Publicar</Button>
                 </div>
               </div>
             </div>
           </div>
     ...
     <Dialog open={showConfirmModal} onOpenChange={setShowConfirmModal}>
       <DialogContent className="sm:max-w-md">
         ...
         <div className="py-4">
           <p className="text-sm text-gray-600">¿Estás seguro de que quieres publicar este comentario?</p>
           <div className="mt-3 p-3 bg-gray-50 rounded-lg">
             <p className="text-sm">{nuevoComentario}</p>
           </div>
         </div>
         <div className="flex justify-end gap-2">
           <Button variant="outline" onClick={() => setShowConfirmModal(false)}>Cancelar</Button>
           <Button onClick={handleComentar} disabled={loading} className="bg-[#441404] hover:bg-[#5a1a05]">Publicar</Button>
         </div>
       </DialogContent>
     </Dialog>
   </>
